...
<section class="chapter">
  ...
  <p class="lead">
    Weniger Ballbesitz heisst: Wenn Thun zum Abschluss kommt, muss er sitzen. Und er sitzt. Mit 7.7 Schüssen pro Tor ist der FC Thun die effizienteste Mannschaft der ganzen Liga — während der Durchschnitt bei 9.2 liegt.
  </p>
</section>
```
...
<div class="visual-shell">
  <div class="moment-card">
    ...
    <div style="font-size:.8rem; font-weight:700; color:#6f6f6f; text-transform:uppercase; letter-spacing:.05em; margin-bottom:10px;">
      Schüsse pro Tor — alle Teams (weniger = effizienter)
    </div>
    {''.join(bar_rows)}
    <div class="table-caption" style="margin-top:12px;">
      Thun erzielte 75 Tore aus 578 Schüssen — als einziges Team unter 8 Schüsse pro Tor und damit Ligabeste.
    </div>
  </div>
</div>
"""))
...
<div class="visual-shell">
  <div class="moment-card">
    ...
    <!-- Liga-Ranking -->
    <div style="font-size:.8rem; font-weight:700; color:#6f6f6f; text-transform:uppercase; letter-spacing:.05em; margin-bottom:10px;">
      Save % — alle Teams (mehr = besser)
    </div>
    {''.join(save_bar_rows)}
    <div class="table-caption" style="margin-top:12px;">
      Steffen hielt in der Saison 103 von 140 Schüssen — 76.4% Fangquote. Nur Lugano liegt knapp höher.
    </div>
  </div>
</div>
"""))
...
<section class="chapter">
  ...
  <p class="lead">
    Thun spielt konstant gut. Aber im Herbst 2025 passiert etwas, das die Saison endgültig kippt: 10 Siege in Folge. Kein einziges Unentschieden, keine einzige Niederlage. Der Schlüsselmoment dieser Meisterschaft — in 10 Spielen.
  </p>
</section>
```
...
streak_df = thun_df.iloc[max_start : max_start + max_streak].copy()

# Build timeline cards
cards = []
for idx, (_, g) in enumerate(streak_df.iterrows()):
    score = f"{g['goals_for']}:{g['goals_against']}"
    venue_label = "Heim" if g["venue"] == "H" else "Auswärts"
    is_first = idx == 0
    is_last = idx == max_streak - 1
    border = "border: 2px solid #E63946;" if (is_first or is_last) else "border: 2px solid transparent;"
    badge = ""
    if is_first:
        badge = '<div style="font-size:.65rem; font-weight:700; color:#E63946; text-transform:uppercase; letter-spacing:.08em; margin-bottom:3px;">Start</div>'
    if is_last:
        badge = '<div style="font-size:.65rem; font-weight:700; color:#E63946; text-transform:uppercase; letter-spacing:.08em; margin-bottom:3px;">Ende</div>'
    cards.append(f"""
      <div style="background:#fff; {border} border-radius:8px; padding:12px 10px; text-align:center; min-width:100px; flex:1;">
        {badge}
        <div style="font-size:.7rem; color:#6f6f6f; margin-bottom:4px;">{g['date']}</div>
        <div style="font-size:.78rem; font-weight:700; color:#202020; margin-bottom:6px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;" title="{g['opponent']}">{g['opponent'].replace('FC ', '').replace('BSC ', '')}</div>
        <div style="font-size:1.4rem; font-weight:900; color:#E63946; line-height:1;">{score}</div>
        <div style="font-size:.65rem; color:#6f6f6f; margin-top:4px;">{venue_label}</div>
      </div>
    """)
...
<div class="visual-shell">
  <div class="moment-card">
    <div style="text-align:center; margin-bottom:24px;">
      <div style="font-size:3rem; font-weight:900; color:#E63946; line-height:1;">{max_streak} Siege</div>
      <div style="font-size:1rem; color:#202020; margin-top:4px;">am Stück — {start_date} bis {end_date}</div>
      <div style="font-size:.88rem; color:#6f6f6f; margin-top:4px;">{total_goals_for} Tore geschossen, {total_goals_against} kassiert</div>
    </div>
    <div style="display:flex; gap:6px; flex-wrap:wrap; justify-content:center; margin-bottom:16px;">
      {''.join(cards)}
    </div>
    <div class="table-caption">
      Jede Karte steht für ein Spiel der Serie. Rot umrandet: Beginn und Ende der längsten Siegesserie der Saison.
    </div>
  </div>
</div>
"""))
...
<section class="chapter">
  ...
  <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px; margin:28px 0;">
    ...
    <div style="background:#f5f2ed; border-left:3px solid #E63946; border-radius:0 6px 6px 0; padding:16px 18px;">
      ...
      <div style="font-size:.93rem; color:#202020;">YB, Basel und Zürich lieferten nie die Konstanz, die man von ihnen erwartet. Thun brauchte keinen Ausrutscher zu provozieren — die Konkurrenz stolperte von selbst.</div>
