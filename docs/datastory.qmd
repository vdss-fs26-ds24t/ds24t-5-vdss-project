...
<style>
  ...
  .closing-image .source {
    margin-top: 10px;
    color: var(--muted);
    font-size: .82rem;
    line-height: 1.4;
  }
  .closing-image .source a {
    color: var(--muted);
  }
  .footer-note {
    max-width: 980px;
...
<div id="scroll-track" style="position:fixed;right:0;top:0;width:12px;height:100vh;background:rgba(0,0,0,0.08);z-index:9999;pointer-events:none;">
  <div id="scroll-bar" style="width:100%;height:0%;background:#E63946;transition:height .05s linear;"></div>
</div>
<script>
(function(){
  function updateBar(){
    var el = document.getElementById('scroll-bar');
    if(!el) return;
    var scrolled = window.scrollY || document.documentElement.scrollTop;
    var total = document.body.scrollHeight - window.innerHeight;
    el.style.height = (total > 0 ? Math.min(scrolled/total*100,100) : 0) + '%';
  }
...
<section class="chapter tight">
  <div class="chapter-head">
    ...
    <h2>Aufsteiger, Aussenseiter, Meister</h2>
  </div>
  <p class="lead">
...
plot_df = df.copy()
plot_df["label"] = plot_df["team_name"] + " (" + plot_df["standings_rank"].astype(str) + ".)"

TEAM_COLORS = {
    "FC Thun":       THUN_RED,
    "FC ST. Gallen": "#006633",
    "FC Winterthur": "#CC0000",
    "FC Sion":       "#D40000",
    "Grasshoppers":  "#0057A8",
    "FC Luzern":     "#003DA5",
    "Lausanne":      "#0066CC",
    "FC Zurich":     "#1A5BA6",
    "Servette FC":   "#670823",
    "BSC Young Boys":"#FFD700",
    "FC Basel 1893": "#C8102E",
    "FC Lugano":     "#1A1A1A",
}
...
fig.update_traces(
    ...
    hovertemplate=(
        "<b>%{customdata[0]}</b><br>"
        "Ballbesitz: %{customdata[1]:.1f}%<br>"
        "Gefährliche Angriffe: %{customdata[2]:.1f}<extra></extra>"
    ),
)
fig.update_layout(
    dragmode=False,
    showlegend=False,
    xaxis=dict(showgrid=True, gridcolor="#eee8de", zeroline=False, title_font=dict(size=16), tickfont=dict(size=13)),
    yaxis=dict(showgrid=True, gridcolor="#eee8de", zeroline=False, title_font=dict(size=16), tickfont=dict(size=13)),
)
fig.add_vline(x=league_avg["sm_ball_possession_pct"], line_dash="dot", line_color="#777")
fig.add_hline(y=league_avg["sm_dangerous_attacks"], line_dash="dot", line_color="#777")
fig.add_annotation(
...
<section class="chapter tight">
  <div class="note">
    Lesart: Die vertikale gestrichelte Linie markiert den Liga-Durchschnitt beim Ballbesitz, die horizontale den Liga-Durchschnitt bei den gefährlichen Angriffen.
...
df_eff = df_eff.merge(df[["team_name", "standings_rank"]], on="team_name", how="left")
...
for _, r in df_eff.iterrows():
    ...
    bar_rows.append(f"""
      <div style="display:flex; align-items:center; gap:10px; margin-bottom:5px;">
        <div style="width:160px; font-size:.85rem; text-align:right; {weight} color:#202020;">{r['team_name']} ({int(r['standings_rank'])}.)</div>
...
<div class="visual-shell">
  <div class="moment-card">
    ...
    <div style="text-align:center; margin-bottom:28px;">
      ...
      <div style="font-size:1rem; color:#202020; margin-top:4px;">Schüsse pro Tor: <strong>Platz 2 in der Super League</strong></div>
    ...
    <div style="background:#f5f2ed; border-radius:8px; padding:18px 20px; margin-bottom:24px;">
      <div style="display:grid; grid-template-columns:90px 1fr; gap:10px 16px; align-items:center;">
        <div style="font-size:.8rem; font-weight:700; color:#E63946; text-transform:uppercase; letter-spacing:.05em; line-height:1.3;">FC Thun<br><span style="font-size:.78rem; color:#6f6f6f; font-weight:400;">{thun_n} Schüsse</span></div>
        <div style="line-height:2.2;">{thun_balls}</div>
        <div style="font-size:.8rem; font-weight:700; color:#6f6f6f; text-transform:uppercase; letter-spacing:.05em; line-height:1.3;">Liga-Ø<br><span style="font-size:.78rem; color:#6f6f6f; font-weight:400;">{league_n} Schüsse</span></div>
        <div style="line-height:2.2;">{league_balls}</div>
      </div>
    </div>
...
df_saves = df_saves.merge(df[["team_name", "standings_rank"]], on="team_name", how="left")
...
fig_radar.update_layout(
    ...
    font=dict(family="Arial, sans-serif", color=INK),
    paper_bgcolor="white",
    margin=dict(l=80, r=80, t=70, b=90),
    height=520,
    title=dict(
...
fig_radar.show(config={"scrollZoom": False, "displayModeBar": False})
```
...
<section class="chapter">
  <div class="chapter-head">
    ...
    <h2>Kein Märchen, eine Formel</h2>
  </div>
  <p class="lead">
...
<section class="chapter">
  <div class="chapter-head">
    ...
    <h2>Das Urteil ist eindeutig: Verdient</h2>
  </div>
...
<figure class="closing-image">
  <img src="pics/7868591741733528.webp" alt="FC Thun feiert den Meistertitel">
  <figcaption class="source">
    Quelle: <a href="https://www.watson.ch/imgdb/46b0/Qx,A,0,0,8010,5340,3337,2225,1335,890/7868591741733528" target="_blank" rel="noopener">Watson</a>
  </figcaption>
</figure>
...
<div class="footer-note">
  ...
  Vereinsinfos: Transfermarkt, Wikipedia. Bilder: Wikimedia Commons (CC BY-SA 3.0), Watson.<br>
</div>
</div>
```
